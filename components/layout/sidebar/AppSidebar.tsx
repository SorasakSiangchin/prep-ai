...
import { getPageIconAndPage } from "@/helpers/helpers";
...
const AppSiderbar = () => {
  ...
  // Key ช่วยให้การจัดการ identifier ของ elements (โดยเฉพาะใน list หรือ components ที่มีการ re-render)
          ...
          {pages.map((page) => (
            <ListboxItem
              ...
              startContent={
                <IconWrapper
                  className={`bg-${
                    getPageIconAndPage(page.path).color
                  }/10 text-${getPageIconAndPage(page.path).color}`}
                >
                  <Icon
                    icon={getPageIconAndPage(page.path)?.icon}
                    className="text-lg "
                  />
                </IconWrapper>
              }
